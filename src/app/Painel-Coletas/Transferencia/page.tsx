...
const TransferenciasPage: React.FC = () => {
  ...
  const tableColumns = useMemo(
    () => buildColetaTableColumns(columns, styles),
    [],
  );
  ...
  return (
    <div className={styles.container}>
      <LoadingOverlay />
      <ColetaPageShell
        title={`${TRANSFERENCIA_TEMPLATE.titulo} - ${empresa?.nomeFantasia?.toUpperCase() ?? ""}`}
        ...
        actions={
          <div className={styles.searchActions}>
            <button className={styles.actionButton} onClick={() => setIsModalOpen(true)} title="Cadastrar nova transferência">
              <span>{TRANSFERENCIA_TEMPLATE.createButtonLabel}</span>
              <FiRepeat size={18} />
            </button>
            <button className={styles.actionButton} onClick={() => table.reload()} title="Atualizar transferências">
              <span>Atualizar</span>
        ...
        renderActions={(row) => (
          <ColetaRowActions
            row={row}
            styles={styles}
            labels={{ delete: "Excluir coleta avulsa", edit: "Editar coleta" }}
      ...
      {codEmpresa && (
        <ModalCadastrarColeta
          ...
          tipoColeta={2}
          titulo={TRANSFERENCIA_TEMPLATE.createModalTitle}
        />
      )}
      {codEmpresa && (
        <ModalEditarColeta
